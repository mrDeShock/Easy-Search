...
if not ADOConnection1.Connected then
  begin
  ShowMessage('База данных не подключена');
  Exit;
  end;
...
with TADOQuery.Create(nil) do // создаем объект для получения данных из БД
  ...
  if not Eof then // если есть хоть одна строка, то Eof не будет ложью. значит есть с чем работать
    ...
    Label6.Caption := 'благодаря Hex';
    Label6.Visible:=true;
    Memo2.Lines.Text := FieldByName('Program').AsString;
    Memo2.Visible:=true;
    Label4.Visible:=true;
    Memo3.Lines.Text := FieldByName('Описание').AsString;
    Memo3.Visible:=true;
    Label5.Visible:=true;
    ...
    if not Eof then
      ...
      Label6.Caption := 'Ничего не найдено. Загрузите, пожалуйста, этот файл на мою почту: mrDeShock@yandex.ua';
      Label6.Visible:=true;
